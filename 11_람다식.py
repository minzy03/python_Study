...
print(sorted(fruits, key=lambda x:x[1])) #x는 리스트내의 각 원소가 대입
print(sorted(fruits, key=lambda x:x[1], reverse=True)) #내림차순 정렬
...
# %%
#랜덤게임
#알파벳 a~z까지 출력(랜덤으로 알파벳 중 하나를 제외해서)
...
randAlpha = random.choice(alphaList)
alphaList.remove(randAlpha)
print(f"알파벳 출력 : {','.join(alphaList)}")
...
for i in alphaList :
    ...
    if user == randAlpha :
        endTime = time.time()
        print(f"정답입니다. 경과시간 : {endTime - startTime:.2f}초")
        break
    else:
