#%%
#Path Intellisense : 경로 입력시 사용 용이함, './' ->현재경로
#(1) 텍스트 파일 읽기
...
#ID가 id02인 사람의 주소를 부산광역시 해운대구로 변경
search_id = 'id02'
...
#a모드(append 모드) : 파일 끝에 내용을 추가하여 파일 쓴다.
#w모드 (write 모드) : 기존 파일 내용을 다 지우고 새롭게 쓴다.
...
with open(os.path.join(path,'대전.csv'), 'r', encoding='euc-kr') as f : #rainbow csv : csv파일 보기좋게
    lines = f.readlines()
    ...
    for line in lines[1:] : #동 정보
        ...
        for gu, dong in zip(guList, dongList) :
            ...
            if not os.path.isfile(file_path) and dong != '':
                f = open(file_path, 'wb') #바이너리파일로 저장할 땐 'wb'->이미지,음악,동영상 등 / 'w'->텍스트파일
                f.close()
